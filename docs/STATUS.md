...
Last reviewed: 2026-04-28 (after baserunning pass — `PHASE_BASERUNNING.md`. Leads, pickoffs, errant pickoff throws with OF backup tags, and steal attempts shipped. Activates first-base coach ratings, pitcher `holdRunners` / `armAccuracy`, catcher `popTime`, runner `stealing` / `baserunningIQ`. Renderer adds per-runner lead offset + idle sway from the new `lead` events; throw arcs for pickoff/backup deferred. 127 tests passing.).
